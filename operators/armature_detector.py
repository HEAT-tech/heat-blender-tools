...
from ..lib import animation_lists
from ..lib import detection_manager
from ..lib import custom_schemes_manager


class DetectFaceShapes(bpy.types.Operator):
    bl_idname = "heat.detect_face_shapes"
    ...
    def execute(self, context):
        ...
        for shape_name_key in animation_lists.face_shapes:
            setattr(obj, 'heat_face_' + shape_name_key, detection_manager.detect_shape(obj, shape_name_key))

        return {'FINISHED'}


class DetectActorBones(bpy.types.Operator):
    bl_idname = "heat.detect_actor_bones"
    bl_label = "Auto Detect"
    ...
    def execute(self, context):
        ...
        for bone_name_key in animation_lists.get_bones().keys():
            setattr(obj, 'heat_actor_' + bone_name_key, detection_manager.detect_bone(obj, bone_name_key))

        return {'FINISHED'}


class SaveCustomShapes(bpy.types.Operator):
    bl_idname = "heat.save_custom_shapes"
    ...
    def execute(self, context):
        ...
        for shape_name_key in animation_lists.face_shapes:
            shape_name_selected = getattr(obj, 'heat_face_' + shape_name_key)
...
class SaveCustomBones(bpy.types.Operator):
    bl_idname = "heat.save_custom_bones"
    ...
    def execute(self, context):
        ...
        for bone_name_key in animation_lists.get_bones().keys():
            bone_name_selected = getattr(obj, 'heat_actor_' + bone_name_key)
...
class SaveCustomBonesRetargeting(bpy.types.Operator):
    bl_idname = "heat.save_custom_bones_retargeting"
    bl_label = "Save Custom Bones"
    bl_description = "This saves the currently selected bones and they will then get automatically detected"
    bl_options = {'INTERNAL'}
...
class ImportCustomBones(bpy.types.Operator, bpy_extras.io_utils.ImportHelper):
    bl_idname = "heat.import_custom_schemes"
...
class ExportCustomBones(bpy.types.Operator, bpy_extras.io_utils.ExportHelper):
    bl_idname = "heat.export_custom_schemes"
...
class ClearCustomBones(bpy.types.Operator):
    bl_idname = "heat.clear_custom_bones"
...
class ClearCustomShapes(bpy.types.Operator):
    bl_idname = "heat.clear_custom_shapes"
